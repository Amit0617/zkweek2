...
//     emulating bridge. first it sends tokens to omnibridge mock then it sends to the pool
    ...
    expect(tornadoPoolBalance).to.be.equal(aliceDepositAmount.sub(aliceBalance))
  })
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    // Alice deposits 0.13 ETH into tornado pool in Layer 1
...
//     const bobWithdrawAmount = utils.parseEther('0.06')
    ...
    // Alice withdraws remaining of his funds from the shielded pool
//     const aliceWithdrawAmount = utils.parseEther('0.07')
    const aliceWithdrawAmount = aliceChangeUtxo.amount
    const [sender, gov, l1Unwrapper, multisig, alice] = await ethers.getSigners()
    const aliceEthAddress = alice.address
    const aliceChangeAgainUtxo = new Utxo({
